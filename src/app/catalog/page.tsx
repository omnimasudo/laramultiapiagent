...
import { Search, Bot, Zap, Terminal, Database, ShieldAlert, ChevronLeft, ChevronRight, Eye, X } from 'lucide-react';
...
function CatalogContent() {
  ...
  const activeCategorySlug = searchParams.get('category'); // e.g. "development"

  // Derive category name from slug
  const activeCategoryName = useMemo(() => {
    if (!activeCategorySlug) return null;
    const category = apisData.categories.find(c => c.slug === activeCategorySlug);
    return category ? category.name : null; // e.g. "Development"
  }, [activeCategorySlug]);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [debouncedSearchQuery, activeCategorySlug]);

  // FIX: Smarter filtering using debounced value to prevent lag
  const filteredApis = useMemo(() => {
    let apisArray = apisData.apis as API[];

    // 1. Filter by Category (if active)
    if (activeCategoryName) {
      apisArray = apisArray.filter(api => api.category === activeCategoryName);
    } 
    // 2. If NO category is active, apply default sorting: Development -> Weather -> Rest
    else {
      apisArray = [...apisArray].sort((a, b) => {
        const catA = a.category || '';
        const catB = b.category || '';
        
        // Priority 1: Development
        if (catA === 'Development' && catB !== 'Development') return -1;
        if (catB === 'Development' && catA !== 'Development') return 1;
        
        // Priority 2: Weather (only if neither is Development)
        // If both are Development, they are equal in this check.
        if (catA === 'Weather' && catB !== 'Weather') return -1;
        if (catB === 'Weather' && catA !== 'Weather') return 1;

        // Default: Keep original order (or sort alphabetically/id if desired, but 0 keeps stable sort)
        return 0;
      });
    }
  ...
  }, [debouncedSearchQuery, activeCategoryName]);
  ...
  const handleClearFilter = () => {
    router.push('/catalog'); // Remove query params
  };
  ...
  return (
    <div className="max-w-7xl mx-auto relative z-10">
      ...
      <div className="mb-12">
        ...
        <p className="font-mono text-cyber-text-light/60 max-w-2xl text-sm leading-relaxed mb-6">
        ...
        {activeCategoryName && (
          <div className="flex items-center gap-4 mb-4 animate-in fade-in slide-in-from-left duration-300">
            <span className="font-mono text-xs text-cyber-text-light/50 uppercase tracking-widest">
               active_filter:
            </span>
            <button 
              onClick={handleClearFilter}
              className="group flex items-center gap-2 px-4 py-2 bg-cyber-neon/10 border border-cyber-neon/50 hover:bg-cyber-neon/20 hover:border-cyber-neon transition-all clip-tactical-sm"
            >
              <span className="font-heading text-cyber-neon font-bold uppercase text-sm">
                Category: {activeCategoryName}
              </span>
              <div className="bg-cyber-neon/20 group-hover:bg-cyber-neon/40 p-1 rounded-full transition-colors">
                <X className="w-3 h-3 text-cyber-neon" />
              </div>
            </button>
          </div>
        )}

        {/* SEARCH BAR (Only show if filtering logic allows user to override, but typically acts as "search within current filter") */}
        <div className="relative group max-w-2xl w-full mb-8">
            ...
            <div className="relative bg-black/80 border border-cyber-border clip-tactical p-2 flex items-center gap-2 focus-within:border-cyber-gold focus-within:shadow-[0_0_10px_rgba(255,215,0,0.2)] transition-all duration-300">
              <Search className="w-5 h-5 text-cyber-text-light/50 ml-3" />
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder={activeCategoryName ? `Search in ${activeCategoryName}...` : "Search API name, description or category..."}
      ...
      <div className="grid grid-cols-1 mb-12">
        {/* Ask Lara (Expanded to full width) */}
...
export default function CatalogPage() {
  return (
    <div className="min-h-screen text-cyber-text-light pt-24 pb-20 px-6 relative overflow-hidden">
      ...
      <Suspense fallback={
        <div className="flex items-center justify-center h-64 text-cyber-neon font-mono animate-pulse">
